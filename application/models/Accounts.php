...
// receipt table
    ...
    public function cash_data() {
        ...
        $cash = 0;
        $datse = date('Y-m-d');
        $this->db->select('transection.*,sum(transection.pay_amount) as amount');
        $this->db->from('transection');
    ...
    public function get_date_wise_closing_report($per_page = null, $page = null, $from_date = null, $to_date= null) {
        if($from_date==null){
    ...
    public function taxdata($from_date,$to_date){
        return $this->db->select('a.*,b.invoice')
                        ...
                        ->where('a.create_by',$this->session->userdata('user_id'))
