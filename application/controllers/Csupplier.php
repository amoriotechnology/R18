      ...
      //  $ext = end(explode('.', $filename));
        ...
        if (($handle = fopen($_FILES['upload_csv_file']['tmp_name'], 'r')) !== FALSE)
        {
         ...
         while($csv_line = fgetcsv($fp,1024)){
           ...
           $insert_csv['phone'] = (!empty($csv_line[2])?$csv_line[2]:'');
           $insert_csv['email'] = (!empty($csv_line[3])?$csv_line[3]:'');
           $insert_csv['emailaddress'] = (!empty($csv_line[4])?$csv_line[4]:'');
           $insert_csv['contact'] = (!empty($csv_line[5])?$csv_line[5]:'');
           ...
           $insert_csv['previous_balance'] = (!empty($csv_line[12])?$csv_line[12]:'');
           $insert_csv['terms'] = (!empty($csv_line[13])?$csv_line[13]:0);
                }
                $depid = date('Ymdis');
                $supplierdata = array(  
                    'created_by'       =>  $this->session->userdata('user_id'),
           'supplier_name'  => $insert_csv['supplier_name'],
            ...
            'paymentterms'       => $insert_csv['terms'],
             'credit_limit'       => $insert_csv['previous_balance'],
            'status'        => 1
                );
